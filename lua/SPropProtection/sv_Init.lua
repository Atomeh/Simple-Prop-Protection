...
function SPropProtection.SetupSettings()
	if sql.TableExists("spropprotection") then
		local testquery = sql.Query("SELECT drive from spropprotection")
		if testquery == false then
			sql.Query("DROP TABLE spropprotection")
			MsgC(
				Color(235, 0, 0), 
				"==================================================\n",
				"---------------Simple Prop Protection-------------\n",
				"==================================================\n",
				"Looks like you were using SPP before.\n",
				"SPP settings table needs to be wiped to make\n",
				"sure this version will function correctly.\n",
				"\n",
				"You may just ignore this message.\n",
				"Only settings like Deletion Delay will be reset.\n",
				"=================================================\n"
				)
		end
	end
...
function SPropProtection.CanDrive(ply, ent)
	...
	if driveConfig  == 0 then return true end
	
	local canDrive = SPropProtection.PlayerCanTouch(ply, ent)

	if not canDrive then return false end

end
hook.Add("CanDrive", "SPropProtection.CanDrive", SPropProtection.CanDrive)
--Restricting context menu also restricts entity driving because it's a part of the context menu.
--I don't know why I added two separate checkboxes.


//Other properties
function SPropProtection.CanProperty(ply, property, ent)
	
	local propertyConfig = tonumber(SPropProtection.Config["property"])

	if propertyConfig == 0 then return true end

	local canProperty = SPropProtection.PlayerCanTouch(ply, ent)
	
	if not canProperty then return false end
...
function SPropProtection.CanTool(ply, tr, mode)
	...
	elseif(mode == "nail") then
		...
		if(tr2.Hit and tr2.Entity:IsValid() and !tr2.Entity:IsPlayer()) then
...
function SPropProtection.EntityTakeDamage(ent, dmginfo)
    ...
    if(not ent:IsValid() or ent:IsPlayer() or not attacker:IsPlayer()) then
		return
	end
	if(!SPropProtection.PlayerCanTouch(attacker, ent)) then
		dmginfo:SetDamage(0)
		timer.Simple(0.1, SPropProtection.EntityTakeDamageFireCheck, ent)
	end
end
...
function SPropProtection.CleanupProps(ply, cmd, args)
	...
	elseif( !ply:IsValid() or ply:IsAdmin()) then
		for k,v in pairs(player.GetAll()) do
...
function SPropProtection.ApplySettings(ply, cmd, args)
	...
	local drive = tonumber(ply:GetInfo("spp_drive") or 1)
	local property = tonumber(ply:GetInfo("spp_property") or 1)
	local delay = math.Clamp(tonumber(ply:GetInfo("spp_delay") or 120), 1, 500)
	
	sql.Query("UPDATE spropprotection SET toggle = "..toggle..
		", admin = "..admin..", use = "..use..", edmg = "..edmg..
		", pgr = "..pgr..", awp = "..awp..", dpd = "..dpd..", dae = "..dae..
		", drive = "..drive..", property = "..property..", delay = "..delay)
